...
void orderFood(customer[],int&,double&,double&,double&);
void totalPayment(customer[],int&,double&,double&);
void confirm(customer[],int&,char[][20],char[][20],char[][35],double&,double&,double&);
...
int main()
{
    ...
    double totalDiscount, price, food;
    ...
    {
        totalDiscount=0, price=0, food=0;
        choice2=mainScreen();
        system("cls");
        if(choice2 == 1)
        {
            ...
            system("cls");
            orderFood(CUSTOMERS,noOfCustomer,totalDiscount,price,food);
            system("COLOR F1");
            CUSTOMERS[noOfCustomer].total=(CUSTOMERS[noOfCustomer].total+totalDiscount)+food-totalDiscount;
            confirm(CUSTOMERS,noOfCustomer,zoneName,roomName,hourName,price,food,totalDiscount);
            system("COLOR 1F");
            cout << "\n\n   Confirm (Y / N): ";
            cin >> choice;
            if(tolower(choice) == 'y')
        ...
        else if(choice2 == 2)
        {
            ...
            while(returnScreen != 1)
            {
                cout << "\n   1. ERROR: PRESS 1 TO RETURN TO MAIN SCREEN!: ";
                cin >> returnScreen;
            }
            system("cls");
        }
...
void booking(customer CUSTOMERS[], int& noOfCustomer)
{
    ...
    while(choice3 == 1 || choice3 == 0)
    {
        ...
        cout << endl;
        if(tolower(CUSTOMERS[noOfCustomer].member) == 'y' || tolower(CUSTOMERS[noOfCustomer].member) == 'n')
        {
             choice3=2;
        }
        ...
        {
            if(tolower(CUSTOMERS[noOfCustomer].member) != 'y' && tolower(CUSTOMERS[noOfCustomer].member) != 'n')
                cout << "   Error: Membership Card - Enter 'Y' or 'N' only!" << endl;
            if(tolower(CUSTOMERS[noOfCustomer].zone) != 'a' && tolower(CUSTOMERS[noOfCustomer].zone) != 'b')
                    cout << "   Error: Zone - Enter 'A' or 'B' only!" << endl;
            if(tolower(CUSTOMERS[noOfCustomer].room) != 'r' && tolower(CUSTOMERS[noOfCustomer].room) != 'v')
                    cout << "   Error: Type of room - Enter 'R' or 'V' only!" << endl;
            if(tolower(CUSTOMERS[noOfCustomer].student) != 'y' && tolower(CUSTOMERS[noOfCustomer].student) != 'n')
                    cout << "   Error: Student Card - Enter 'Y' or 'N' only!" << endl;
            if(tolower(CUSTOMERS[noOfCustomer].birthday) != 'y' && tolower(CUSTOMERS[noOfCustomer].birthday) != 'n')
                    cout << "   Error: Birthday Special - Enter 'Y' or 'N' only!" << endl;
            if(choice3 != 2)
            {
                cout << "\n   Press 1 to re-enter information!: ";
                cin >> choice3;
                while(choice3 != 1)
                {
                    cout << "\n   Press 1 !!!!: ";
...
void orderFood(customer CUSTOMERS[], int& noOfCustomer,double& totalDiscount,double& price,double& food)
{
    int choice4=1;
    cout << "\n\t\t\t           ORANGE BOX KARAOKE" << endl;
    cout << "\t\t           Opening hour: 10:00 AM - 4:00 AM" << endl;
...
void confirm(customer CUSTOMERS[],int& noOfCustomer,char zoneName[][20],char roomName[][20],char hourName[][35],double& price,double& food, double& totalDiscount)
{
    cout << "\n\t\t\t           ORANGE BOX KARAOKE" << endl;
    cout << "\t\t           Opening hour: 10:00 AM - 4:00 AM" << endl;
    cout << "   " << setw(74) << setfill('-') << "-" << endl;
